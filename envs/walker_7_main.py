...
class ModularEnv(MujocoEnv, gym_utils.EzPickle):
    ...
    def step(self, a):
        posbefore = self.data.qpos[0]; self.do_simulation(a, self.frame_skip)
        posafter  = self.data.qpos[0]; torso_height, torso_ang = self.data.qpos[1:3]
        reward = (posafter - posbefore) / self.dt; reward += 1.0; reward -= 1e-3 * float(np.square(a).sum())
        terminated = not (torso_height > 0.8 and torso_height < 2.0
